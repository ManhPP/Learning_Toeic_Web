...
@section('title','update part 6')
@section("css")
    <link rel="stylesheet" type="text/css" href="{{ URL::asset("css/admin-update-part6/admin-update-part6.css") }}">
@endsection
@section('navbar')
    @parent
@endsection
    ...
    <div style="position: relative; top: 6em">
        ...
        <div
            ...
            <div class="header-content">
                <div style="padding-left: 1em">Practic part 6 (12 sentences)</div>
            </div>
            <div class="content">
                <div class="p1">
                    <div class="direct">
                    ...
                    <div class="list-cau">
                        ...
                        @foreach($partDoc->part6Paragraphs  as $doanVan)
                            <div class="boundary-para" data-id="{{$doanVan->id }}">
                                <p class="ques refer-ques">Questions {{$index }}- {{($index+2)}}
                                    ...
                                    <div class="ques" data-asw="{{$cau->daDung }}" data-id="{{$cau->id }}">
                                        ...
                                        <div class="row">
                                            ...
                                            <label class="col-12 col-md-6"><input
                                                        disabled="disabled" type="radio" name="choise{{$cau->id }}s1"
                                                        {{$checkA }} value="A"><span class="asw-content">{{$cau->daA }}</span></label>
                                            <label class="col-12 col-md-6"><input
                                                        disabled="disabled" type="radio" name="choise{{$cau->id }}s1"
                                                        {{$checkB}} value="B"><span class="asw-content">{{$cau->daB }}</span></label>
                                            <label class="col-12 col-md-6"><input
                                                        disabled="disabled" type="radio" name="choise{{$cau->id }}s1"
                                                        {{$checkC }} value="C"><span class="asw-content">{{$cau->daC }}</span></label>
                                            <label class="col-12 col-md-6"><input
                                                        disabled="disabled" type="radio" name="choise{{$cau->id }}s1"
                                                        {{$checkD }} value="D"><span class="asw-content">{{$cau->daD }}</span></label>

                                        </div>
                                        @if($index % 3 ==0)
                                            <div class="align-right">
                                                <img class="ico-modified ico-del" data-id="{{$doanVan->id }}"
                                                     alt="minus"
                    ...
                    <div style="text-align: center;">
                        <button id="submit">Update part 6</button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <table class="table table-bordered" style="min-width: 400px">
                            ...
                            @foreach($arrDoan as $doan)
                                <tr class="d-flex row" data-id="{{$doan->id }}">
                                    <td class="col-12"><input class="checkbox-choise"
                                                              type="checkbox"> <span class="ques-content">Đoạn
												văn id {{$doan->id }}</span> <img class="expand-ico"
...
@section('footer')
    @parent
    <script src="{{URL::asset("js/admin-update-part6/admin-update-part6.js")}}"></script>
@endsection
